...
void TestQ1_add2(CuTest *tc) {
	...
	int n = 6;
	double input1[6] = {1.20, 0.78, 5.00, 11.00, 0.02, 0.03};
	double input2[6] = {0.80, 0.22, 3.00, 0.90, 1.00, 3.30};
	double actual[6];
	double expected [6] = {2.00, 1.00, 8.00, 11.90, 1.02, 3.33};
	add_vectors(input1,input2,actual,n);
	int i;
	for (i=0; i<n; i++)
		CuAssertDblEquals(tc, expected[i], actual[i],0.009);
...
void TestQ1_scalar_prod2(CuTest *tc) {
	...
	int n = 4;
	double input1[4] = {1.20, 4.20, 6.11, 0.50};
	double input2[4] = {1.25, 2.20, 3.22, 2.00};
	double expected=31.414 ;
	double actual = scalar_prod(input1,input2,n);
...
void TestQ1_norm2(CuTest *tc) {
	...
	int n = 5;
	double input1[5] = {8.29, 3.18, 11.00, 1.00, 0.69};
	double expected=14.188 ;
    double actual = norm2(input1,n);
...
void TestQ1_add_boundary(CuTest *tc) {
    ...
    int i;
    for (i=0; i<n; i++)
        CuAssertDblEquals(tc, expected[i], actual[i],0);
}
...
void TestQ2_false2(CuTest *tc) {
	...
	int input[3][3] = {{-2, 2, 1},{ 1, 3, 2}, {1, -2, 0}};
	int actual = is_diag_dom(input);
	int expected = 0;
	CuAssertIntEquals(tc, expected, actual);
}
...
void TestQ3_2(CuTest *tc) {
	int n = 3;
	int input[3][3] = {{0, 2, 4},{ 1, 3, 5},{ 10, 15, 20}};
	int expected[9]= {0, 1, 2, 10, 3, 4, 15, 5, 20};
	int actual[9];
	diag_scan(input,actual);
...
void TestQ5_shorter2(CuTest *tc) {
	...
	const char input[] =  "wawa";
	char actual[20]={0};
	string_copy(input,actual,3);
	char expected[20] = "wa";
	CuAssertStrEquals(tc, expected, actual);
}
//===========================================================
...
void TestQ6_combined2(CuTest *tc) {
	int n=8;
	int input[]={0,69,9,29,0,0,0,31};
	int actual_output_val[8]={0};
	int actual_output_pos[8]={0};
	int actual_output[8]={0};
	efficient(input,actual_output_val,actual_output_pos,n);
	reconstruct(actual_output,8,actual_output_val,actual_output_pos,4);
	int expected_output[8]= {0,69,9,29,0,0,0,31};
...
void TestQ6_2(CuTest *tc) {
	int n=8;
	int input[]={1,2,3,0,-7,0,0,48};
	int actual_output_val[8]={0};
	int actual_output_pos[8]={0};
	efficient(input,actual_output_val,actual_output_pos,n);
	int expected_output_val[8]={ 1, 2, 3,-7,48,0,0,0};
	int expected_output_pos[8]={0, 1, 2,4,7,0,0,0};
